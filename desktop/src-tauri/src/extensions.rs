...
//!
//! The first two extensions are the developer-only  pair (the CLI,
//! hosted in a terminal the app owns, and Desktop, launched as a separate
//! application), but nothing in this module is -specific: any
...
/// The two developer-only  extensions. The CLI is hosted in a
/// terminal the app owns (ConPTY on Windows) and launched with 's own
...
const BUILTIN: [Manifest; 2] = [
    Manifest {
        ...
        lookups: &[
            Lookup::OnPath(""),
            Lookup::UnderHome(&[".config/manicode/"], "codecane"),
        ],
        launch: &["", "--cwd", "{project}"],
        install: "npm install -g ",
        note: "Runs the official  CLI in a terminal here. Your account, ads, and backend stay 's; nurb only hosts the session.",
    },
    Manifest {
        id: "-desktop",
        label: " Desktop",
        ...
        lookups: &[Lookup::UnderLocalAppData(&["Programs/", ""], "")],
        launch: &[],
        install: "https://.com/desktop",
        note: "Launches the official  Desktop app. nurb cannot open projects in it yet: Desktop has no documented open-project mechanism.",
    },
];
...
mod tests {
    ...
    fn manifest_lookup_is_exact() {
        ...
        assert!(manifest("-desktop").is_some());
    ...
    fn terminal_manifest_launches_with_cwd_only() {
        ...
        assert_eq!(m.launch, &["", "--cwd", "{project}"]);
        assert!(m.dev_only);
    }
    ...
    fn enabled_state_persists_and_rejects_unknown_ids() {
        ...
        assert!(!ext.is_enabled("-cli")); // dev-only: off by default
        ext.set_enabled("-cli", true).unwrap();
        assert!(ext.is_enabled("-cli"));
        assert!(ext.set_enabled("nope", true).is_err());
        drop(ext);
        let reloaded = Extensions::load(&dir);
        assert!(reloaded.is_enabled("-cli"));
        std::fs::remove_dir_all(&dir).unwrap();
    }
}
