# Written permission request to  (draft)

Status: draft, not sent. Address: support@.com.

This is the written-confirmation request the integration research
(`docs/windows/-INTEGRATION.md`, section 27) requires before the
 developer extensions can move out of developer-only status. Until a
...
"Third-party terminal hosting of the  CLI - permission request".
...
tool) and want to let its users start the official  CLI from inside
...
1. **Terminal panel hosting.** The app discovers the user's own official
    CLI install (npm install -g , or the downloaded binary)
...
3. **Nurb MCP server (user-enabled).** Separately, we ship a local MCP server
   ...
   add it to their own  `mcp.json` if they want; we never edit
    configuration. Tool calls happen only inside a session the human
   started.

4. **External launch of  Desktop.** A button that launches the
   official  Desktop app installed by the user, with no window
   manipulation and no project targeting (we understand none is documented).

What we do not do: we do not bundle, modify, patch, or recompile any 
binary; we do not call 's backend directly; we do not automate,
chain, or headlessly run  sessions; we do not suppress, filter, or
redirect 's telemetry or ads; we do not spoof location or identity;
we do not create accounts.

Questions we would like confirmed:

1. Is hosting the official  CLI in a third-party terminal panel, with
   byte-only transport as described, acceptable as normal use of the
   official product?
2. May the app use the  name (text only) in its developer extension
...
3. Is a user-configured nurb MCP server in the user's own mcp.json
   ...
   it manually?
4. Is externally launching  Desktop acceptable?
