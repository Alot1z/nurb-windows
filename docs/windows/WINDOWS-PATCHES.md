...
## Extension system (developer-only Freebuff pair)
...
- `desktop/src-tauri/src/extensions.rs`: generic extension registry (manifests, declarative lookups, enable/disable state, version gate). Freebuff CLI and Freebuff Desktop are the first two entries, both developer-only and disabled by default.
...
- The Freebuff pair is developer-only and off by default until the written-permission questions in `docs/windows/FREEBUFF-INTEGRATION.md` section 27 are answered; nothing in the extension system itself depends on that.
